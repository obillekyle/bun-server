...
export class SyncHelper {
  ...
  static getStringSimilarity(str1: string, str2: string) {
  ...
  static findBestMatchAndPrompt(
    ...
    for (const newCamel of unmappedSet) {
      const score = SyncHelper.getStringSimilarity(oldName, Case.snake(newCamel))
  ...
  private static initDbTablesMap(dbConstraints: SyncTypes.DBConstraints) {
  ...
  private static tableHasTransform(tsTableObj: any): boolean {
    return (
  ...
  private static resolveOldTableMapping(
    plan: SyncPlan,
    dbTables: any,
  ...
  private static handleTableRenames(
    plan: SyncPlan,
    ...
    for (const [newCamel, tsTableObj] of Object.entries(constraints)) {
      ...
      const hasTransform = SyncHelper.tableHasTransform(tsTableObj)
      if (hasTransform) plan.tablesToRebuild.add(Case.snake(newCamel))
      SyncHelper.resolveOldTableMapping(plan, dbTables, unmappedDbTables, newCamel, tsTableObj, hasTransform)
    }
    return unmappedDbTables
  }

  private static promptAndRenameTables(
    plan: SyncPlan,
    dbTables: any,
    unmappedDbTables: Set<string>,
    logger: any,
    ...
    for (const oldCamel of [...unmappedDbTables]) {
      ...
      const bestMatch = SyncHelper.findBestMatchAndPrompt(
        dbName,
  ...
  private static diffColumnMismatch(
    plan: SyncPlan,
    dbName: string,
    camelCol: string,
    tsCol: any,
  ...
  private static resolveColumnRenames(
    plan: SyncPlan,
    camelTable: string,
    dbName: string,
    constraints: any,
  ...
  private static resolveUnmappedDbCols(
    plan: SyncPlan,
    camelTable: string,
    dbName: string,
    logger: any,
    ...
    for (const oldDbCol of [...unmappedDbCols]) {
      const bestMatch = unmappedTsCols.size
        ? SyncHelper.findBestMatchAndPrompt(oldDbCol, unmappedTsCols, 'column', dbName, logger)
        : null
      if (bestMatch) {
        plan.columnsToRename.push({
          table: dbName,
  ...
  private static diffTableColumns(
    plan: SyncPlan,
    camelTable: string,
    dbName: string,
    constraints: any,
    ...
    SyncHelper.resolveColumnRenames(plan, camelTable, dbName, constraints, unmappedDbCols, unmappedTsCols, existingDbCamelCols)
    SyncHelper.resolveUnmappedDbCols(plan, camelTable, dbName, logger, unmappedDbCols, unmappedTsCols, existingDbCamelCols)
    ...
    for (const camelCol of existingDbCamelCols) {
      ...
      if (tsCol && dbCol) {
        SyncHelper.diffColumnMismatch(plan, dbName, camelCol, tsCol, dbCol, MESSAGES)
      }
    }
  }

  private static diffViewStrings(
    plan: SyncPlan,
    camelTable: string,
    dbName: string,
    constraints: any,
  ...
  private static diffTableViewsAndColumns(
    plan: SyncPlan,
    dbTables: any,
    constraints: any,
    logger: any,
    ...
    for (const camelTable of Object.keys(dbTables)) {
      ...
      if (
        ...
        continue
      if (SyncHelper.diffViewStrings(plan, camelTable, dbName, constraints)) continue
      SyncHelper.diffTableColumns(plan, camelTable, dbName, constraints, logger, MESSAGES)
    }
  }

  static async buildSyncPlan(
    adapter: DBAdapter,
    constraints: SyncTypes.DBConstraints,
    logger: any,
    MESSAGES: any,
    ...
    const dbTables = SyncHelper.initDbTablesMap(plan.dbConstraintsForDiff)
    const unmappedDbTables = SyncHelper.handleTableRenames(plan, constraints, dbTables)

    SyncHelper.promptAndRenameTables(plan, dbTables, unmappedDbTables, logger)
    SyncHelper.diffTableViewsAndColumns(plan, dbTables, constraints, logger, MESSAGES)
  ...
  static async processTableRebuild(
    tx: DBAdapter,
    table: string,
    constraints: SyncTypes.DBConstraints,
  ) {
  ...
  private static async dropIndexesPhase(
    tx: DBAdapter,
    indexesToDrop: Set<string>,
    MESSAGES: any,
  ) {
  ...
  private static updateTableRefsAfterRename(plan: SyncPlan, oldName: string, newName: string) {
  ...
  private static async renameTablesPhase(tx: DBAdapter, plan: SyncPlan, MESSAGES: any) {
    for (const { oldName, newName } of plan.tablesToRename) {
      ...
      SyncHelper.updateTableRefsAfterRename(plan, oldName, newName)
    }
  }

  private static async renameColumnsPhase(
    tx: DBAdapter,
    plan: SyncPlan,
    MESSAGES: any,
  ) {
  ...
  private static async dropTablesPhase(tx: DBAdapter, plan: SyncPlan, MESSAGES: any) {
  ...
  private static async dropColumnsPhase(tx: DBAdapter, plan: SyncPlan, MESSAGES: any) {
  ...
  private static async addColumnsPhase(tx: DBAdapter, plan: SyncPlan, MESSAGES: any) {
  ...
  private static async rebuildTablesPhase(
    tx: DBAdapter,
    plan: SyncPlan,
    constraints: SyncTypes.DBConstraints,
    MESSAGES: any,
    ...
    for (const table of plan.tablesToRebuild) {
      ...
      await SyncHelper.processTableRebuild(tx, table, constraints)
    }
  }

  private static async syncViewsAndTablesPhase(
    tx: DBAdapter,
    constraints: SyncTypes.DBConstraints,
    MESSAGES: any,
  ) {
  ...
  private static async addIndexesPhase(
    tx: DBAdapter,
    indexesToAdd: Map<string, SyncTypes.IndexConstraint>,
    MESSAGES: any,
  ) {
  ...
  static async executeSyncPlan(
    ...
    await SyncHelper.dropIndexesPhase(tx, indexesToDrop, MESSAGES)
    await SyncHelper.renameTablesPhase(tx, plan, MESSAGES)
    await SyncHelper.renameColumnsPhase(tx, plan, MESSAGES)
    await SyncHelper.dropTablesPhase(tx, plan, MESSAGES)
    await SyncHelper.dropColumnsPhase(tx, plan, MESSAGES)
    await SyncHelper.addColumnsPhase(tx, plan, MESSAGES)
    await SyncHelper.rebuildTablesPhase(tx, plan, constraints, MESSAGES)
    await SyncHelper.syncViewsAndTablesPhase(tx, constraints, MESSAGES)
    await SyncHelper.addIndexesPhase(tx, indexesToAdd, MESSAGES)
  }

  static hasOldWrappers(constraints: SyncTypes.DBConstraints) {
